...
import type { IActionContext, MetadataQuads, MetadataBindings } from '@comunica/types';
...
describe('QuerySourceHypermedia', () => {
  ...
  describe('A QuerySourceHypermedia instance', () => {
    ...
    beforeEach(() => {
      source = new QuerySourceHypermedia(10, 'firstUrl', 'forcedType', 64, undefined, mediators, logWarning);
    });

    describe('getSelectorShape', () => {
      it('should return a selector shape', async() => {
        expect(await source.getSelectorShape(context)).toEqual({
          type: 'operation',
          operation: {
    ...
    describe('toString', () => {
      it('should return a string representation', async() => {
        expect(source.toString()).toEqual('QuerySourceHypermedia(firstUrl)');
      });
    });
    ...
    describe('queryBindings', () => {
      ...
      it('should match three chained sources', async() => {
        ...
        source = new QuerySourceHypermedia(10, 'firstUrl', 'forcedType', 64, undefined, mediatorsThis, logWarning);
        await expect(source.queryBindings(operation, context)).toEqualBindingsStream([
          BF.fromRecord({
            s: DF.namedNode('s11'),
            p: DF.namedNode('p11'),
      ...
      it('should emit an error when mediatorQuerySourceIdentifyHypermedia rejects', async() => {
        ...
        await expect(source.queryBindings(operation, context).toArray()).rejects.toThrowError(`mediatorQuerySourceIdentifyHypermedia error`);
      });
    });

    describe('queryQuads', () => {
      it('should throw', async() => {
        expect(await source.queryQuads(<any> undefined, context).toArray())
          .toBeRdfIsomorphic([
            quad('s1', 'p1', 'o1'),
            quad('s2', 'p2', 'o2'),
          ]);
    ...
    describe('queryBoolean', () => {
      it('should throw', async() => {
        expect(await source.queryBoolean(<any> undefined, context))
          .toEqual(true);
      });
    });
    ...
    describe('getSource', () => {
      it('should get urls based on mediatorQuerySourceIdentifyHypermedia', async() => {
        expect(await source.getSource({ url: 'startUrl' }, {}, context)).toMatchObject({
          link: { url: 'startUrl' },
      ...
      it('should get urls based on mediatorQuerySourceIdentifyHypermedia without dataset id', async() => {
        ...
        source = new QuerySourceHypermedia(10, 'firstUrl', 'forcedType', 64, undefined, mediatorsThis, logWarning);
        expect(await source.getSource({ url: 'startUrl' }, {}, context)).toEqual({
          link: { url: 'startUrl' },
          handledDatasets: {},
          metadata: { a: 1 },
      ...
      it('should apply the link transformation', async() => {
        ...
        expect(await source.getSource({ url: 'startUrl', transform }, {}, context)).toEqual({
          link: { url: 'startUrl', transform },
          handledDatasets: { MYDATASET: true },
          metadata: { a: 1 },
          source: expect.anything(),
        });
        expect(transform).toHaveBeenCalledWith(expect.any(require('stream').Readable));
      });
      ...
      it('should apply the link context', async() => {
        ...
        expect(await source.getSource({ url: 'startUrl', context: new ActionContext({ a: 'b' }) }, {}, context))
          .toEqual({
        ...
        expect(mediatorMetadata.mediate).toHaveBeenCalledWith({
          quads: expect.any(require('stream').Readable),
          triples: true,
          url: 'startUrl',
          context: new ActionContext({
            a: 'b',
      ...
      it('should delegate dereference errors to the source', async() => {
        ...
        source = new QuerySourceHypermedia(10, 'firstUrl', 'forcedType', 64, undefined, mediatorsThis, logWarning);
        const ret = await source.getSource({ url: 'startUrl' }, {}, context);
        expect(ret).toEqual({
          link: { url: 'startUrl' },
          handledDatasets: { MYDATASET: true },
  ...
  describe('A QuerySourceHypermedia instance with aggregated store', () => {
    let source: QuerySourceHypermedia;
    let aggregateStore: StreamingStoreMetadata;

    beforeEach(() => {
      aggregateStore = new StreamingStoreMetadata(
        undefined,
        async(accumulatedMetadata, appendingMetadata) => <MetadataBindings>
          (await mediators.mediatorMetadataAccumulate.mediate({
            mode: 'append',
            accumulatedMetadata,
            appendingMetadata,
            context,
      ...
      source = new QuerySourceHypermedia(10, 'firstUrl', 'forcedType', 64, aggregateStore, mediators, logWarning);
    });
    ...
    describe('queryBindings', () => {
      it('should match three chained sources when queried multiple times in parallel', async() => {
        ...
        source = new QuerySourceHypermedia(10, 'firstUrl', 'forcedType', 64, aggregateStore, mediatorsThis, logWarning);
