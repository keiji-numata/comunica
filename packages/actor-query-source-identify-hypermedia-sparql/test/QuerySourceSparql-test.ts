...
describe('QuerySourceSparql', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    source = new QuerySourceSparql('http://example.org/sparql', ctx, mediatorHttp, 'values', false, 64, 10);
  });
  ...
  "results": { 
    ...
    it('should not cache if cache is disabled', async() => {
      source = new QuerySourceSparql('http://example.org/sparql', ctx, mediatorHttp, 'values', false, 0, 10);
  ...
  "results": { 
    "bindings": [
      ...
      source = new QuerySourceSparql('http://example.org/sparql', ctx, thisMediator, 'values', true, 64, 10);
      await expect(source.queryBindings(AF.createPattern(DF.namedNode('s'), DF.variable('p'), DF.namedNode('o')), ctx))
        .toEqualBindingsStream([
          BF.fromRecord({
            p: DF.namedNode('p1'),
