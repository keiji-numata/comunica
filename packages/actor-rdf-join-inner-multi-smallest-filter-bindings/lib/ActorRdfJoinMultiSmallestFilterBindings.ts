...
export class ActorRdfJoinMultiSmallestFilterBindings extends ActorRdfJoin {
  ...
  protected async getOutput(action: IActionRdfJoin): Promise<IActorRdfJoinOutputInner> {
    ...
    const sourceWrapper: IQuerySourceWrapper = secondIn.operation.metadata?.scopedSource;
    const second: IJoinEntry = {
      output: {
        type: 'bindings',
        bindingsStream: sourceWrapper.source.queryBindings(
          secondIn.operation,
          sourceWrapper.context ? action.context.merge(sourceWrapper.context) : action.context,
          { filterBindings: { bindings: smallestStream1.clone(), metadata: first.metadata }},
        ),
  ...
  protected async getJoinCoefficients(
    ...
    if (!second.operation.metadata?.scopedSource) {
      throw new Error(`Actor ${this.name} can only process if entries[1] has a source`);
    }
    const sourceWrapper: IQuerySourceWrapper = second.operation.metadata?.scopedSource;
