...
export class ActorContextPreprocessQuerySourceIdentify extends ActorContextPreprocess {
  ...
  public async run(action: IAction): Promise<IActorContextPreprocessOutput> {
    ...
    // Rewrite sources
    if (context.has(KeysQueryOperation.querySourcesUnidentified)) {
      const querySourcesUnidentified: QuerySourceUnidentified[] = action.context
        .get(KeysQueryOperation.querySourcesUnidentified)!;
      ...
      context = action.context
        .delete(KeysQueryOperation.querySourcesUnidentified)
